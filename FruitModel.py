...
class NaiveBayesModel:
    def __init__(self):
        ...
        self.token_num = [defaultdict(int), defaultdict(int)] # token在正负样本中出现次数
        self.V = 0 #语料库token数量
    ...
    def count(self):
        # TODO: YOUR CODE HERE
        # 提示：统计token分布不需要返回值
        for i in tqdm(range(self.dataset.len)):
            tokens, label = self.dataset[i]
    ...
    def __call__(self, text):
        # TODO: YOUR CODE HERE
        # 返回1或0代表当前句子分类为正/负样本
        sum_labels = sum(self.pos_neg_num)

        token_num = [ 0 for i in self.token_num ]
        for i in range(len(token_num)):
            token_num[i] = sum(list( self.token_num[i].values() ))
        assert sum(token_num) == self.V

        probability = [ 0 for i in self.token_num ]
        for idx in range(len(probability)):
            py = self.pos_neg_num[idx] / sum_labels
            px = 1000

            for token in text:
                tmp = (self.token_num[idx][token] + 1) / (token_num[idx] + 1) # Add-one smooth
                px *= tmp
            
            probability[idx] = py * px
...
class Embedding():
    ...
    def __call__(self, text, max_len=50):
        # TODO: YOUR CODE HERE
        D = 100
        ret = np.zeros((max_len, D), dtype=np.float32)
        ...
        keys = self.emb.keys()
        for token in text:
            if token in keys and i < max_len:
                ret[i, :] = self.emb[token]
                i += 1
        return ret
