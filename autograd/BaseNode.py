...
class Attention(Node):
    ...
    def cal(self, X):
        ...
        weight = np.matmul(Q, K.transpose(( 0, 2, 1 ))) / np.sqrt(self.dim)

        exp_w = np.exp( weight - np.max(weight, axis=-1, keepdims=True) )
        softmax_score = exp_w / np.sum(exp_w, axis=-1, keepdims=True)

        self.cache.append(( X, softmax_score, Q, K, V ))
        ret = np.matmul( softmax_score, V )
        return ret
...
class Linear(Node):
    ...
    def backcal(self, grad):
        ...
        self.grad.append( X.swapaxes(-2, -1) @ grad.reshape(( -1, grad.shape[-1])) ) # grad of weight
        self.grad.append( np.sum(grad.reshape(( -1, grad.shape[-1] )), axis=0) ) # grad of bias
...
class ResLinear(Node):
    ...
    def backcal(self, grad):
        ...
        self.grad.append( X.swapaxes(-1, -2) @ grad.reshape(( -1, grad.shape[-1] )) )
        self.grad.append( np.sum(grad.reshape(( -1, grad.shape[-1] )), axis=0) )
        return grad @ (self.params[0].T + np.identity(dim))
...
class LogSoftmax(Node):
    ...
    def cal(self, X):
        ...
        tmp_X = X - X_max
        exp_X = np.exp(tmp_X)
        exp_sum_X = np.sum(exp_X, axis=self.dim, keepdims=True)
        mean_exp_X = exp_X / exp_sum_X

        self.cache.append(mean_exp_X)
        ret = tmp_X - np.log( exp_sum_X )
        return ret

    def backcal(self, grad):
        # TODO: YOUR CODE HERE
        mean_exp_X = self.cache[-1]
        one = np.ones_like( mean_exp_X )
        dim = one.shape[self.dim]

        return grad @ (np.identity(dim) - one.T @ mean_exp_X)
