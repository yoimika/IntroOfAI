...
def entropy(Y: np.ndarray):
    ...
    num = Y.shape[0]
    rate = featcnt / num 
    return - np.sum( rate * np.log(rate) )
...
def gain(X: np.ndarray, Y: np.ndarray, idx: int):
    ...
    # TODO: YOUR CODE HERE
    H_D = entropy( Y )
    H_A = np.array([entropy( Y[feat == item] ) for item in ufeat])

    ret = H_D - np.dot( featp, H_A )
    return ret
...
def buildTree(X: np.ndarray, Y: np.ndarray, unused: List[int], depth: int, purity_bound: float, gainfunc: Callable, prefixstr=""):
    ...
    # TODO: YOUR CODE HERE
    # prefixstr += f'{root.featidx}'
    for item in ufeat:
        idx1 = (feat == item)

        node = buildTree(X[idx1], Y[idx1], unused, depth-1, purity_bound, gainfunc, prefixstr)
