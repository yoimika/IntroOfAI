...
def q3(name_score_dict):
    ...
    res = []
    for name, value in name_score_dict.items():
        res.append(Student(name, value))
    res.sort(key=lambda student: student.score, reverse=True)
    return res
...
def q5():
    ...
    dis = np.sqrt(x**2 + y**2)
    res = 4 * np.count_nonzero(dis <= 1.0)/np.size(dis)
    return res
...
class Layout:
    ...
    def recover_layoutText(self):
        ...
        """
        layoutText = np.chararray((self.height, self.width))
        layoutText[:] = ' '

        wall_idx = np.asarray(self.walls).T
        food_idx = np.asarray(self.foods).T

        layoutText[wall_idx[0], wall_idx[1]] = '%'
        layoutText[food_idx[0], food_idx[1]] = '.'
        layoutText[self.pacman_pos[0], self.pacman_pos[1]] = 'P'
        res = '\n'.join([''.join(item) for item in layoutText.astype(np.str_).tolist()])
        return res
