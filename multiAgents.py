    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent):
            ...
            for action in actions:
                tmp_state = state.generateSuccessor( index_of_agent, action )
                tmp_res   = minimizer( tmp_state, depth, 1 )
                if ( value < tmp_res[0] ):
                    value      = tmp_res[0]
                    maxiAction = action
        ...
        def minimizer(state, depth, index_of_agent):
            ...
            for action in actions:
                tmp_state = state.generateSuccessor( index_of_agent, action )
                tmp_res   = minimizer( tmp_state, depth, index_of_agent+1 ) if (index_of_agent+1) < num_agents \
                      else  maximizer( tmp_state, depth-1, 0 )
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent, alpha, beta):
            ...
            for action in actions:
                tmp_state = state.generateSuccessor( index_of_agent, action )
                tmp_res   = minimizer( tmp_state, depth, 1, alpha, beta )
                if value < tmp_res[0]:
                    value = tmp_res[0]
                    maxiAction = action
                if alpha < tmp_res[0]:
        ...
        def minimizer(state, depth, index_of_agent, alpha, beta):
            ...
            for action in actions:
                tmp_state = state.generateSuccessor( index_of_agent, action )
                tmp_res   = minimizer( tmp_state, depth, index_of_agent+1, alpha, beta ) if (index_of_agent+1) < num_agents \
                      else  maximizer( tmp_state, depth-1, 0 , alpha, beta )
                if value > tmp_res[0]:
                    value = tmp_res[0]
                    miniAction = action
                if beta > tmp_res[0]:
...
"""
PS
这里 value 返回的值不能受 alpha 和 beta 影响，不能一开始就给 value 赋值为 alpha 和 beta
同理，alpha 和 beta 都不能受 value 的影响
这些都是为了避免刚开始进入一个节点更新错误。
"""
...
class MCTSAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState, mcts_time_limit=10):
        ...
        def Selection(cgs, cgstree : Node):
            '''
                ...
                for action in legal_actions:
                    if action == 'Stop':
                        if cgstree.stop == None:
                            ...
                            children.append(( cgstree.stop, action ))
                    elif action == 'North':
                        if cgstree.north == None:
                            flag = False
                            break
                        else:
                            children.append(( cgstree.north, action ))
                    elif action == 'South':
                        if cgstree.south == None:
                            flag = False
                            break
                        else:
                            children.append(( cgstree.south, action ))
                    elif action == 'East':
                        if cgstree.east == None:
                            flag = False
                            break
                        else:
                            children.append(( cgstree.east, action ))
                    elif action == 'West':
                        if cgstree.west == None:
                            flag = False
                            break
                        else:
                            children.append(( cgstree.west, action ))
                
                if flag == False:
                    break

                best_state, best_action = best_UCT( children )
        ...
        def Expansion(cgstree : Node):
            ...
            '''
            for action in legal_actions:
                tmp_node = Node( (cgstree.statevalue.generateSuccessor(0, action), 0, 1) )
                tmp_node.parent = cgstree
                if action == 'Stop':
                    cgstree.stop = tmp_node
                elif action == 'North':
                    cgstree.north = tmp_node
                elif action == 'South':
                    cgstree.south = tmp_node
                elif action == 'East':
                    cgstree.east = tmp_node
                elif action == 'West':
                    cgstree.west = tmp_node
                else:
                    raise Exception('Unrecognized action encountered')
